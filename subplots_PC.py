...
for edge_type in influence_type:
    for dataset in generate_edge_types.dataset_list:
        image_files.append(f'plots/{dataset}/TUMF/{dataset}_{edge_type}_pc_out.png')
...
plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1, wspace=3, hspace=1)
plt.tight_layout()
plt.savefig('plots/subplots_PC.png')
plt.show()
