...
for i,j in enumerate(dataset_list):
    for k,h in enumerate(dataset_list):
        ...
        TM_cosine_similarity_df.at[j,h] = consine_sim(TM_array[i,:], TM_array[k,:])
TM_cosine_similarity_df.to_csv('Data/preprocessed/TM_cosine_similarity.csv')
TM_cosine_similarity_df = TM_cosine_similarity_df.round(2)
...
for i,j in enumerate(dataset_list):
    for k,h in enumerate(dataset_list):
        ...
        TF_cosine_similarity_df.at[j,h] = consine_sim(TF_array[i,:], TF_array[k,:])
...
for i,j in enumerate(dataset_list):
    for k,h in enumerate(dataset_list):
        ...
        UM_cosine_similarity_df.at[j,h] = consine_sim(UM_array[i,:], UM_array[k,:])
...
for i,j in enumerate(dataset_list):
    for k,h in enumerate(dataset_list):
        ...
        UF_cosine_similarity_df.at[j,h] = consine_sim(UF_array[i,:], UF_array[k,:])
